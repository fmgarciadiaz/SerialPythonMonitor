    ...
    def run(self):
        ...
        while self.running:
            ...
            if chunk:
                ...
                # Protección anti-lag: si el buffer acumula más de 25.000 bytes,
                ...
                for line_bytes in lines[:-1]:
                    ...
                    if is_header_line(parts):
                        ...
                        for p in parts:
                            lp = p.lower()
                            if "v_in" in lp or lp == "vin":
                                normalized_headers.append("V_IN")
                            elif "v_out" in lp or lp == "vout":
                                normalized_headers.append("V_OUT")
                            elif "adc_in" in lp or lp == "adcin":
                                normalized_headers.append("ADC_IN")
                            elif "adc_out" in lp or lp == "adcout":
                                normalized_headers.append("ADC_OUT")
                            elif "tiempo" in lp:
                                normalized_headers.append("Tiempo (us)")
                            elif "muestra" in lp:
                                normalized_headers.append("Muestra")
                            # "estado", "adc" genérico y cualquier campo desconocido
                            # se ignoran silenciosamente (no se agregan como columna)
