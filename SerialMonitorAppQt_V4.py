...
class SerialWorker(QtCore.QObject):
    ...
    Específicamente adaptado para formato 2 Canales V2:
    ...
    def __init__(self, port: str, baud: int):
        ...
        self.columns = ["V_IN", "V_OUT", "ADC_IN", "ADC_OUT", "Muestra", "Tiempo (us)"]
    ...
    def run(self):
        ...
        while self.running:
            ...
            if chunk:
                ...
                # =====================================================
                # PROTOCOLO BINARIO DEL ARDUINO
                #
                # Cabecera de bloque:
                #   4 bytes : DATA
                #   1 byte  : estado (1 = ON, 0 = OFF)
                #   2 bytes : cantidad de muestras (uint16 LE)
                #
                # Cada muestra:
                #   4 bytes : timestamp (uint32 LE)
                #   2 bytes : ADC_IN   (uint16 LE)
                #   2 bytes : ADC_OUT  (uint16 LE)
                #
                # Total por muestra = 8 bytes
                # =====================================================

                while True:
                    # Buscar sincronización "DATA".
                    pos = self.raw_buffer.find(b"DATA")

                    if pos < 0:
                        # "DATA" puede haber quedado cortado entre dos
                        # lecturas. Conservamos los últimos 3 bytes.
                        if len(self.raw_buffer) > 3:
                            self.raw_buffer = self.raw_buffer[-3:]
                        break

                    # Eliminar cualquier byte basura anterior a DATA.
                    if pos > 0:
                        del self.raw_buffer[:pos]

                    # Cabecera completa todavía no disponible.
                    if len(self.raw_buffer) < 7:
                        break

                    estado = self.raw_buffer[4]
                    n = int.from_bytes(
                        self.raw_buffer[5:7],
                        byteorder="little",
                        signed=False,
                    ...
                    # Validación básica para evitar que un byte corrupto
                    # haga crecer indefinidamente el tamaño esperado.
                    if estado not in (0, 1) or n == 0 or n > 10000:
                        del self.raw_buffer[0]
                        continue

                    block_size = 7 + n * 8

                    # El bloque todavía no llegó completo.
                    if len(self.raw_buffer) < block_size:
                        break

                    # Extraer el bloque completo.
                    block = self.raw_buffer[:block_size]
                    del self.raw_buffer[:block_size]
                    ...
                    for i in range(n):
                        time_us = int.from_bytes(
                            block[offset:offset + 4],
                            byteorder="little",
                            signed=False,
                        )

                        adc_in = int.from_bytes(
                            block[offset + 4:offset + 6],
                            byteorder="little",
                            signed=False,
                        )

                        adc_out = int.from_bytes(
                            block[offset + 6:offset + 8],
                            byteorder="little",
                            signed=False,
                        )

                        # El Arduino usa analogReadResolution(10):
                        # ADC = 0..1023, referencia = 3.3 V.
                        v_in = adc_in * 3.3 / 1023.0
                        v_out = adc_out * 3.3 / 1023.0
                        ...
                        sample = {
                            ...
                            "Muestra": float(i),
                            "Tiempo (us)": float(time_us),
                        }

                        batch.append(sample)
                        offset += 8

            # Emitir lotes con la misma lógica de actualización rápida
            # que tenía el programa original.
    ...
    def _parse_sample_parts(self, parts: List[str]) -> Optional[Dict[str, float]]:
        """
        Interpreta filas de entrada.
        Caso principal V2: estado, muestra, tiempo_us, ADC_IN, V_IN, ADC_OUT, V_OUT
        """
        # 1. Caso estándar de 7 columnas (estado, muestra, tiempo_us, ADC_IN, V_IN, ADC_OUT, V_OUT)
        if len(parts) >= 7:
            try:
                muestra = float(parts[1])
                tiempo_us = float(parts[2])
                adc_in = float(parts[3])
                v_in = float(parts[4])
                adc_out = float(parts[5])
                v_out = float(parts[6])
                return {
                    "V_IN": v_in,
                    "V_OUT": v_out,
        ...
        # 2. Caso de 6 columnas sin estado (muestra, tiempo_us, ADC_IN, V_IN, ADC_OUT, V_OUT)
        if len(parts) == 6:
            try:
                muestra = float(parts[0])
                tiempo_us = float(parts[1])
                adc_in = float(parts[2])
                v_in = float(parts[3])
                adc_out = float(parts[4])
                v_out = float(parts[5])
                return {
                    "V_IN": v_in,
                    "V_OUT": v_out,
        ...
        # 3. Caso de 5 columnas (estado, muestra, tiempo_us, ADC_IN, ADC_OUT) con cálculo de voltajes
        if len(parts) == 5:
            try:
                muestra = float(parts[1])
                tiempo_us = float(parts[2])
                ...
                adc_out = float(parts[4])
                return {
                    "V_IN": adc_in * 3.3 / 4095.0,
                    "V_OUT": adc_out * 3.3 / 4095.0,
                    "ADC_IN": adc_in,
                    "ADC_OUT": adc_out,
                    "Muestra": muestra,
                    "Tiempo (us)": tiempo_us,
        ...
        # 4. Caso compatible V1 de 4 columnas (estado, muestra, tiempo_us, adc)
        if len(parts) == 4:
            try:
                muestra = float(parts[1])
                tiempo_us = float(parts[2])
...
class SerialMonitorWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.series: Dict[str, deque] = {
            "V_IN": deque(maxlen=MAX_BUFFER_SAMPLES),
            "V_OUT": deque(maxlen=MAX_BUFFER_SAMPLES),
            "ADC_IN": deque(maxlen=MAX_BUFFER_SAMPLES),
            "ADC_OUT": deque(maxlen=MAX_BUFFER_SAMPLES),
    ...
    def _build_ui(self):
        ...
        top_group = QtWidgets.QGroupBox("CONEXIÓN DE ENTRADA (V2: 2 CANALES)")
    ...
    def _rebuild_column_controls(self, names: List[str]):
        ...
        for idx, name in enumerate(names):
            ...
            color = CHANNEL_COLORS.get(name, PALETTE[idx % len(PALETTE)])
    ...
    def _generate_demo_samples(self):
        ...
        for i in range(50):
            ...
            adc_in = int(v_in * 4095.0 / 3.3)
            adc_out = int(v_out * 4095.0 / 3.3)
